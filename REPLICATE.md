...
# Comment transport replay and stacked validation

The human wanted the adversarial comment review resolved, the branch stacked on PR #66, and the live replay findings documented before another review.

- Rebased onto PR #66 (`alejoacelas/saved-api-capture-tests`) at `bb62e716f40acf91e43c9fe6fce64d038fa08629`, fetched from its local worktree. **PR #66 is a merge prerequisite**: this branch relies on its Unicode offset mapping, ID-first tab resolution, and duplicate-title refusal. The previous comment implementation and journal commits became `d6e86a8` and `d8a8b27`.
- Native and Drive comment creation now use a fresh, authenticated transport with a guard at the connection's `request()` boundary. httplib2 may attempt its internal reconnect, but cannot send a second request; lost responses stop with exit 1 and no fallback or revision retry. Token refresh uses the original auth transport before the comment send; a 401 does not replay the comment. Drive server errors and missing IDs also instruct the caller to inspect comments before retrying.
- Added the review's offline lost-response reproducer using real `HttpRequest.execute`, `AuthorizedHttp`, and httplib2 request handling, replacing only socket connections and responses. Both a subsequent 403 and a revision rejection remain unread after the first saved write. Ten transport tests cover native/Drive writes, `RemoteDisconnected`, `ResponseNotReady`, authentication rejection, separate send budgets, token refresh, and uncertain Drive responses. Disabling the guard makes all five selected replay regressions fail.
- JSON/plain non-anchored success now includes `reason: preview_unavailable` or `reason: no_quote`; errors retain `ERR:` on stderr. README and command help document uniqueness, segment/tab scope, refusal and retry rules, and exit codes 0/1/2/3. Fallback omits tab/segment scope because it creates no anchor. Same-body duplicates still refuse with both ranges and advice to use a longer quote or select a tab.
- Added comment-level regressions for case expansion, ID/title collisions, duplicate tab titles, and public exit codes. Kept PR #66's non-destructive native-gap matching for comments; adapted its four captured-document tests to provide a revision and assert the typed anchor result.
- Validation: **1,710 tests passed** with `uv run pytest`; the focused comment suite passed **167 tests**, including **19 new cases** in this follow-up. Full Ruff has **196 diagnostics**, exactly matching `origin/main` by file/code/message counts, with zero additions or removals. The no-stubs gate and `git diff --check` pass. The full pytest run was the normal suite, not a global socket-denial run; the wire regressions explicitly deny socket connections.
- Supplied live replays use `campaign/harness/toolchain-prcomments.json`, pinned to `26ef285d479425fa028a1c255462b4f15ee27951` (Python 3.12.13; lock hash `3f24c878833cc96d`). Runs `20260912T192611-447c6050d5` and `20260912T192550-559045cf57` confirmed revision-bump recovery and a header-segment anchor. Run `20260912T192505-d41fdb95f8` confirmed NBSP matching: Google stores the quoted NBSP HTML-escaped as `&#160;`, which is expected server-side representation, not a failed match. Run `20260912T192518-45b856378f` refused two body matches with exit 3. These are the coordinator's earlier live results; this follow-up did not rerun them or change that toolchain pin.
...
The user wanted PR #68 restacked on #66 `54346b1`: preserved both log sides and all code/tests; 2,986 offline tests passed, no-stubs and ancestry/merge-tree passed, Ruff matched origin/main at 196 findings (0 added/removed). Agent session ctx_098f9adc04f1 · Commits 0698e99e9d97fccb72381069a452878d48cead98.
...
The user wanted PR #68 restacked on #66 `8e5fe26`: preserved both log histories and all code/tests; 3,101 offline tests passed, no-stubs and ancestry/merge-tree passed, Ruff matched origin/main at 196 findings (0 added/removed). Agent session ctx_187d4d2013d6 · Commits a990eb804db3cb8595250eb8277535cef2f3faad.
